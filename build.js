...
(async function main() {
  ...
  /* v1.16+ (audit BUG-AUDIT-ST-5 2026-05-06) : minify avec Terser pour
     diviser la taille du bundle (~228 KB → ~110 KB) et améliorer le boot
     mobile 3G. Garde sourcemap inline pour debug. */
  try {
    const terser = require('terser');
    const minifyResult = await terser.minify(bundle, {
      compress: {
    ...
    if (minifyResult.code) {
      bundle = minifyResult.code;
    }
  } catch (e) {
    console.warn('  ⚠ Terser non disponible, bundle non minifié :', e.message);
  }
